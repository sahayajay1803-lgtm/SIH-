...
FastAPI foundation for SIH26130. The rules engine is the eligibility authority; GLM-5.3 Cloud only assists with intake and explanations.
...
3. Add your GLM Cloud API key to `.env` as `GLM_API_KEY=...`.
...
Explanation requests use the local retrieval layer in `app/services/rag.py` to add source-labelled context for the selected approval before calling GLM Cloud. The current chunks are illustrative placeholders; replace them with verified official material before production use.

...
ollama = OllamaClient(settings.ollama_base_url, settings.ollama_model, settings.ollama_timeout_seconds, settings.ollama_max_concurrency)
...
Use only the supplied profile and approval facts. Do not change whether it applies, invent legal thresholds, or provide legal advice.
Mention that the source is illustrative when it is illustrative. Return plain text in under 120 words."""
...
async def intake(request: IntakeChatRequest) -> IntakeChatResponse:
    ...
    try:
        raw = await ollama.generate(prompt, INTAKE_SYSTEM)
        data = json.loads(raw)
        return IntakeChatResponse.model_validate(data)
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=502, detail="Ollama returned a non-JSON intake response") from exc
...
async def explain(request: ExplanationRequest) -> ExplanationResponse:
    ...
    context = {"profile": request.profile.model_dump(mode="json"), "approval": approval.model_dump(mode="json")}
    try:
        explanation = await ollama.generate(json.dumps(context), EXPLANATION_SYSTEM)
...
async def explain_stream(request: ExplanationRequest) -> StreamingResponse:
    ...
    if not approval:
        raise HTTPException(status_code=404, detail="Approval is not in the illustrative rule set")
    context = json.dumps({"profile": request.profile.model_dump(mode="json"), "approval": approval.model_dump(mode="json")})
    try:
        stream = await ollama.generate(context, EXPLANATION_SYSTEM, stream=True)
